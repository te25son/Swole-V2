...
from swole_v2.security import create_access_token
...
class TestUsers(APITestBase):
    async def test_login_succeeds(self) -> None:
        ...
        response = await self.client.post("/api/v2/auth/login", data=dict(username=user.username, password=password))
    ...
    async def test_login_with_invalid_creds_fails(self) -> None:
        ...
        response = await self._post_error("auth/login", data=dict(username=user.username, password=password))
    ...
    async def _post_success(self, endpoint: str, **kwargs: Any) -> SuccessResponse:
        response = SuccessResponse(**(await self.client.post(f"/api/v2/{endpoint}", **kwargs)).json())
        assert response.code == "ok"
        return response

    async def _post_error(self, endpoint: str, **kwargs: Any) -> ErrorResponse:
        response = ErrorResponse(**(await self.client.post(f"/api/v2/{endpoint}", **kwargs)).json())
